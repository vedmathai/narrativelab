...
class StateNode(AbstractNode):
    ...
    def narrative_relationships(self):
        return [self._narrative_graph.id2state_relationships(id) for id in self.narrative_relationship_ids()]
    ...
    def from_dict(val, narrative_graph):
        ...
        state_node.set_id(val['id'])
        state_node.set_narrative_graph(narrative_graph)
        state_node.set_canonical_name(val['canonical_name'])
        state_node.set_narrative_relationship_ids(val['narrative_relationship_ids'])
        return state_node
